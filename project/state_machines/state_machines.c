...
State *sm_add_state(StateMachine *sm, char *state_name) {
  ...
  if (sm->state_capacity == sm->num_states) {
    printf("Full capacity for states.");
    return NULL;
  }
  ...
  for (int i = 0; i < sm->num_states; i++) {
    if (strcmp(sm->states[i]->name, state_name) == 0) {
      printf("%s is already taken", state_name);
      return NULL;
    }
  }
  ...
  sm->states[sm->num_states - 1] = new_state;
  sm->num_states += 1;
...
Transition *sm_add_transition(StateMachine *sm, char *transition_name,
  ...
  if (sm->num_transitions == sm->transition_capacity) {
    printf("Full capacity for transitions.");
    return NULL;
  }
  ...
  for (int i = 0; i < sm->num_transitions; i++) {
    if (strcmp(sm->transitions[i]->name, origin_state_name) == 0) {
      origin_state = sm->transitions[i];
    }
    if (strcmp(sm->transitions[i]->name, destination_state_name) == 0) {
      destination_state = sm->transitions[i];
    }
  }
  ...
  if (origin_state && destination_state) {
    ...
    sm->transitions[sm->num_transitions - 1] = transition;
    sm->num_transitions += 1;
    return transition;
  }

  // Otherwise, print an error and return NULL
  print("Something went wrong, please check your origin and destination states");
  return NULL;
...
State *sm_do_transition(StateMachine *sm, char *transition_name) {
  ...
  printf("Something went wrong, please check the state machine's current state and also your transition name");
  return NULL;
}
